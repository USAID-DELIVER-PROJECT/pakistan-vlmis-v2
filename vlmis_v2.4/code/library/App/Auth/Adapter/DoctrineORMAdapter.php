...
use App\Auth\Adapter\AbstractAdapter,
    App\Auth\Condition\DoctrineORMCondition,
...
class DoctrineORMAdapter extends AbstractAdapter implements AuthAdapter {
     ...
     */
    public function __construct(EntityManager $em, $entityClass, $entityAlias, $identityColumn, array $conditions = array()
    ) {
        $this->em = $em;
    ...
    public function authenticate() {
        ...
        try {
            return $this->createResult(
                            Zend_Auth_Result::SUCCESS, $query->getSingleScalarResult()
            );
        } catch (NoResultException $e) {
            return $this->createResult(
                            Zend_Auth_Result::FAILURE_IDENTITY_NOT_FOUND, null, array('A record with the supplied identity could not be found.')
            );
        } catch (NonUniqueResultException $e) {
            return $this->createResult(
                            Zend_Auth_Result::FAILURE_IDENTITY_AMBIGUOUS, null, array('More than one record matches the supplied identity.')
            );
        }
    }
    ...
    private function getQuery() {
        ...
        $qb->select(sprintf('%s.%s', $this->getEntityAlias(), $this->getIdentityColumn()))
    ...
    private function addQueryCondition($key, $condition) {
        ...
        if ($condition instanceof DoctrineORMCondition) {
            $qb->andWhere($this->getWhereClause($key, $condition->getCondition()));
