...
<table>
    <form name="filter_prov" action="" method="post">
        <tr>
            ...
            <td id="ajaxSummary" colspan="9" height></td>
        </tr>
    </form>
    <?php
    $where = '';
    while ($row = mysql_fetch_object($rs)) {
        ...
        $strSql = "SELECT DISTINCT
						...
						Province.pk_id AS ProvId,
						District.pk_id AS DistId,
						Tehsil.pk_id AS TehsId,
						UCs.pk_id AS UCId
					FROM
						users
					...
					LEFT JOIN locations AS Tehsil ON UCs.parent_id = Tehsil.pk_id
	...
	?>
    <tr>
        <td colspan="12" style="background-color: #d0e9c6; "><h3><?php echo $result->stkname; ?>s</h3></td>
    </tr>
    <?php
    ...
    <tr>
        ...
        <td>
        	<a onclick="window.open('update-user.php?user=<?= $result->userId ?>', '_blank', 'scrollbars=1,width=350,height=315')" href="javascript:void(0);">
			<?php
		...
		if(!empty($result->ccm_wh_id))
		{
			$qry = "SELECT
					...
					WHERE
						warehouse_population.warehouse_id = " . $result->wh_id;
			$qryRes = mysql_fetch_array(mysql_query($qry));
			$whPop = $qryRes['facility_total_pouplation'];
		}
		?>
        <td <?php echo (!empty($result->ccm_wh_id) && empty($whPop)) ? 'bgcolor="#FF0000"' : '';?>>
        <?php echo $whPop;?>
        </td>
        <?php
		if($result->lvl == 2)
		...
		$qry = "SELECT
				...
				WHERE
					location_populations.location_id = $locId
				AND DATE_FORMAT(location_populations.estimation_date, '%Y') = '2015'";
		$qryRes = mysql_fetch_array(mysql_query($qry));
		?>
        <td <?php echo (!empty($qryRes['population'])) ? '' : 'bgcolor="#FF0000"';?>>
        	<?php echo $qryRes['population'];?>
        </td>
        <td><?php echo $result->wh_id . "-" . $result->locid; ?></td>
        <td><?php
			if (!empty($result->ccm_wh_id)) {
			?>
                <a onclick="window.open('update-ccm-whid.php?wh_id=<?= $result->wh_id ?>&user_id=<?= $result->userId ?>', '_blank', 'scrollbars=1,width=300,height=305')" href="javascript:void(0);">
                <?= $result->ccm_wh_id; ?>
                </a>
			<?php
			} else {
