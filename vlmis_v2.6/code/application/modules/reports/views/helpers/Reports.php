...
class Zend_View_Helper_Reports extends Zend_View_Helper_Abstract {
    ...
    public function reportConsumption($in_type, $rpt_date, $in_loc, $in_item, $in_stk) {
        $row = $this->_em_read->getConnection()->prepare("Call REPgetConsumption('$in_type','$rpt_date','$in_loc','$in_item','$in_stk')");
        $row->execute();
        return $row->fetchAll();
    }
    ...
    public function reportAmc($in_type, $rpt_date, $in_loc, $in_item, $in_stk) {
        ...
        $row = $this->_em_read->getConnection()->prepare("Call REPgetAMC('$in_type','$rpt_date','$in_loc','$in_item','$in_stk')");
        $row->execute();
        return $row->fetchAll();
    }
    ...
    public function reportCb($in_type, $rpt_date, $in_loc, $in_item, $in_stk) {
        ...
        $row = $this->_em_read->getConnection()->prepare("Call REPgetCB('$in_type','$rpt_date','$in_loc','$in_item','$in_stk')");
        $row->execute();
        return $row->fetchAll();
    }
    ...
    public function reportData($in_col, $in_rg, $in_type, $month, $year, $in_item, $in_stk, $in_prov, $in_dist) {
        ...
        $row = $this->_em_read->getConnection()->prepare("SELECT REPgetData('$in_col','$in_rg','$in_type'," . $month . "," . $year . "," . $in_item . ",'$in_stk','$in_prov','$in_dist') AS Value from DUAL");
        $row->execute();
        return $row->fetchAll();
    }
    ...
    public function getReportData($in_type, $in_reporting_date, $in_location, $in_item, $in_stk) {
        ...
        $row = $this->_em_read->getConnection()->prepare("CALL REPgetData('$in_type', '$in_reporting_date', '$in_location', '$in_item', '$in_stk')");
        $row->execute();
        return $row->fetchAll();
    }
    ...
    public function getConsumptionAVG($t, $sel_month, $sel_year, $sel_item, $row_item, $in_prov, $in_dist) {
        ...
        $row = $this->_em_read->getConnection()->prepare("SELECT REPgetConsumptionAVG('" . $t . "'," . $sel_month . "," . $sel_year . ",'" . $sel_item . "','" . $row_item . "'," . $in_prov . "," . $in_dist . ") As Rate from DUAL");
        $row->execute();
        return $row->fetchAll();
    }
    ...
    public function getMOS($t, $sel_month, $sel_year, $sel_item, $row_item, $in_prov, $in_dist) {
        ...
        $row = $this->_em_read->getConnection()->prepare("SELECT REPgetMOS('" . $t . "'," . $sel_month . "," . $sel_year . ",'" . $sel_item . "','" . $row_item . "'," . $in_prov . "," . $in_dist . ") As Rate from DUAL");
    ...
    public function getCB($t, $sel_month, $sel_year, $sel_item, $row_item, $in_prov, $in_dist) {
        ...
        $row = $this->_em_read->getConnection()->prepare("SELECT REPgetCB('" . $t . "'," . $sel_month . "," . $sel_year . ",'" . $sel_item . "','" . $row_item . "'," . $in_prov . "," . $in_dist . ") As Rate from DUAL");
        $row->execute();
        return $row->fetchAll();
    }
     ...
     * @param type $in_dist
     */
    public function getProvincialYearlyReport($report_indicator, $str_date, $in_prov, $in_dist, $in_div) {
        $col_name = "";
        $level = "";
        $prov_clause = "";
        $dist_clause = "";
        ...
        //If province and division filters are selected and district filter is not selected
        if (!empty($in_prov) && !empty($in_div) && empty($in_dist)) {
            $level = '3,4,5';
            //for division
            $level1 = '3';
            $joins = "  INNER JOIN locations ON warehouses.location_id = locations.pk_id
                        INNER JOIN locations AS tehsil ON locations.parent_id = tehsil.pk_id
                        INNER JOIN locations AS dist ON tehsil.parent_id = dist.pk_id
                        INNER JOIN locations AS `div` ON dist.parent_id = `div`.pk_id ";
            $div_clause = "AND `div`.pk_id = $in_div ";
        } else {
            $joins = "";
        }
        if ($in_dist == "") {
        ...
        //Report Indicator: Stock on Hand
		...
		WHERE
			warehouses. STATUS = 1
		AND warehouses.stakeholder_office_id IN ($level)
		AND DATE_FORMAT(
	...
	A.pk_id";
        ...
        //Report Indicator: Received
        else if ($report_indicator == 3) {

            $startDate = date('Y-m-01', strtotime($str_date));
            $endDate = date('Y-m-t', strtotime($str_date));
		...
		INNER JOIN item_pack_sizes ON stakeholder_item_pack_sizes.item_pack_size_id = item_pack_sizes.pk_id
		WHERE
			warehouses.stakeholder_office_id = $level1
        ...
        //Report Indicator: Issue/Dispatch
    ...
    public function getDistrictsYearlyReport($report_indicator, $str_date, $in_prov, $sel_dist, $sel_prod) {
        ...
        if ($report_indicator == 1) {
            ...
            $col_name = 'SUM(hf_data_master.issue_balance) AS total';
            $level = '=6';
        } else if ($report_indicator == 10) {
            $col_name = 'SUM(hf_data_master.issue_balance) + SUM(hf_data_master.wastages) AS total';
            $level = '=6';
        } else if ($report_indicator == 2) {
            $col_name = 'SUM(hf_data_master.closing_balance) AS total';
            $level = '> 2';
            $where = 'AND warehouses.stakeholder_office_id > 3 AND warehouses.stakeholder_office_id < 6';
        ...
        } else if ($report_indicator == 5) {
            $col_name = 'SUM(hf_data_master.closing_balance) AS total';
            $level = '= 4';
            $where = 'AND warehouses.stakeholder_office_id > 3 AND warehouses.stakeholder_office_id < 5';
            if (isset($sel_dist) && !empty($sel_dist)) {
                $con = "AND warehouses.district_id <>  2 AND warehouses.district_id <> $sel_dist";
            } else {
                $con = "AND warehouses.district_id <>  2";
            }
        } else if ($report_indicator == 6) {
            $col_name = 'SUM(hf_data_master.closing_balance) AS total';
            $level = '> 3';
            $where = 'AND warehouses.stakeholder_office_id > 3 AND warehouses.stakeholder_office_id < 6';
            if (isset($sel_dist) && !empty($sel_dist)) {
        ...
        } else if ($report_indicator == 7) {
            $col_name = 'SUM(hf_data_master.closing_balance) AS total';
            $level = '> 4';
            $where = 'AND warehouses.stakeholder_office_id > 4 AND warehouses.stakeholder_office_id < 6';
            if (isset($sel_dist) && !empty($sel_dist)) {
                $con = "AND warehouses.district_id <>  2 AND warehouses.district_id <> $sel_dist";
            } else {
                $con = "AND warehouses.district_id <>  2";
            }
        } else if ($report_indicator == 8) {
            $col_name = 'SUM(hf_data_master.closing_balance) AS total';
            $level = '= 4';
            $where = 'AND warehouses.stakeholder_office_id > 3 AND warehouses.stakeholder_office_id < 6';
            if (isset($sel_dist) && !empty($sel_dist)) {
            ...
            $col_name = 'SUM(hf_data_master.received_balance) AS total';
            $level = '=4';
            $where = "AND stock_master.transaction_type_id = 1";
        } else if ($report_indicator == 4) {

            $col_name = 'SUM(hf_data_master.issue_balance) AS total';
            $level = '=4';
            $where = "AND stock_master.transaction_type_id = 2";
        } else if ($report_indicator == 9) {

            $col_name = 'SUM(hf_data_master.wastages) AS total';
            $level = '=6';
        }

        if ($report_indicator == 3 || $report_indicator == 4) {
            if (isset($sel_dist) && !empty($sel_dist)) {

                $str_qry = "SELECT SUM(A.total) as total,
            A.district_id from ( SELECT
            ABS(SUM(stock_detail.quantity))*item_pack_sizes.number_of_doses AS total,
		...
		DATE_FORMAT(
			...
			AND DATE_FORMAT(
				...
				'%Y-%m-%d'
                    ...
                    AND item_pack_sizes.pk_id = $sel_item

                    $prov_clause
                        AND teh.district_id = $sel_dist
                    GROUP BY locations.pk_id  ) A GROUP BY A.district_id
                    ";
            } else {
                $str_qry = "SELECT SUM(A.total) as total,
            A.district_id from (
            SELECT ABS(SUM(stock_detail.quantity))*item_pack_sizes.number_of_doses AS total,
            warehouses.district_id
            FROM
            stock_master
            INNER JOIN transaction_types ON stock_master.transaction_type_id = transaction_types.pk_id
		...
		DATE_FORMAT(
			...
			AND DATE_FORMAT(
				...
				'%Y-%m-%d'
                    ...
                    AND item_pack_sizes.pk_id = $sel_item
                    $prov_clause
                    GROUP BY warehouses.district_id ) A GROUP BY A.district_id";
            }
        } else if ($report_indicator == 6) {
            if (isset($sel_dist) && !empty($sel_dist)) {
            ...
            WHERE
                    ...
                    AND DATE_FORMAT(stock_master.transaction_date, '%Y-%m') <= '" . $str_date . "'
                    AND item_pack_sizes.pk_id = $sel_item

                    $prov_clause
                        AND teh.district_id = $sel_dist
                    GROUP BY teh.pk_id  ) A GROUP BY A.district_id
                    ";
            } else {
                $str_qry = "SELECT SUM(A.total) as total,
            A.district_id from (
        ...
        } else if ($report_indicator == 6 || $report_indicator == 7) {
            ...
            WHERE
                    ...
                    AND teh.district_id = $sel_dist
                    GROUP BY
            teh.pk_id) A GROUP BY A.district_id
                    ";
            } else {
                $str_qry = "SELECT SUM(A.total) as total,
            A.district_id from (
            ...
            WHERE
                    ...
                    AND item_pack_sizes.pk_id = $sel_item
                    $prov_clause
                    GROUP BY warehouses.district_id  UNION "
                        . "SELECT
            ...
            WHERE
                    item_pack_sizes.item_category_id <> 3
                    AND warehouses.status = 1
                    AND warehouses.stakeholder_office_id > 3 AND warehouses.stakeholder_office_id < 6
                    AND DATE_FORMAT(stock_master.transaction_date, '%Y-%m') <= '" . $str_date . "'
                    AND item_pack_sizes.pk_id = $sel_item
                    $prov_clause
                    GROUP BY warehouses.district_id  UNION "
...
IFNULL(ROUND(
		(
			in_time_reported / reported
		) * 100
	), 0) AS  timelinessPercentage
                           from (
                        SELECT
                                District.pk_id AS districtId,
                                District.location_name AS districtName,
              ...
              LEFT JOIN (
                ...
                AND DATE_FORMAT(
                    ...
                    AND DATE_FORMAT( hf_data_master.created_date, '%d' ) <= '10'
                AND hf_data_master.issue_balance IS NOT NULL
                AND hf_data_master.issue_balance != 0
                GROUP BY
                        District.pk_id
    ...
    public function getReportedDistrictsByUser($wh_type, $provFilter, $sel_dist) {
        ...
        if ($wh_type == 2) {
            if ($provFilter == 'all') {
                ...
                $qry = " AND locations.province_id= '" . $provFilter . "'";
            }
            $str_qry = "SELECT
            ...
            if ($sel_dist == 'all') {
                $qry = " locations.province_id= '" . $provFilter . "'";
                $qry1 = "locations.province_id = $provFilter";
            } else {
                $qry = "warehouses.district_id = $sel_dist";
                $qry1 = "locations.district_id = $sel_dist";
            }
            $str_qry = "SELECT
                    B.pk_id districtId,
    ...
    public function getReportedDistrictsByUser1($wh_type, $provFilter, $sel_dist, $str_date) {
        if ($wh_type == 2) {
            if ($provFilter == 'all') {
                ...
                $qry = "AND warehouses.province_id= '" . $provFilter . "'";
            }
            $str_qry = "
                        SELECT
                                locations.location_name AS districtName,
    ...
    public function getReportedDistrictsByFacility($wh_type, $sel_prov, $sel_dist) {
        if ($wh_type == 2) {
            if ($sel_prov == 'all') {
                ...
                $qry = "AND locations.province_id= '" . $sel_prov . "'";
            }
            ...
            if ($sel_dist == 'all') {
                $qry = "locations.province_id= '" . $sel_prov . "'";
                $qry1 = "locations.province_id= '" . $sel_prov . "'";
            } else {
                $qry = " locations.district_id = $sel_dist";
                $qry1 = " locations.district_id = $sel_dist";
            }
            $str_qry = "SELECT
        B.pk_id districtId,
    ...
    public function getReportedDistrictsByFacility1($wh_type, $sel_prov, $sel_dist, $str_date) {
        ...
        if ($wh_type == 2) {
            if ($sel_prov == 'all') {
                ...
                $qry = " AND warehouses.province_id = '" . $sel_prov . "'";
            }
            $str_qry = "
                SELECT
                        locations.location_name AS districtName,
...
";
    ...
    public function getShipmentDistrict1($provFilter, $distFilter, $date) {
        $str_qry = "SELECT
                ...
                FROM (
                    ...
                    INNER JOIN stock_master ON warehouses.pk_id = stock_master.from_warehouse_id OR warehouses.pk_id = stock_master.to_warehouse_id
                    INNER JOIN warehouses AS towh ON stock_master.to_warehouse_id = towh.pk_id
                    INNER JOIN pilot_districts ON towh.district_id = pilot_districts.district_id
                    WHERE
                        stakeholders.geo_level_id = 4
                        AND warehouses.status = 1
                        AND stock_master.draft = 0
    ...
    public function getShipmentTransactionIssue($whId, $date) {
        $querypro = "SELECT
                        ...
                        DATE_FORMAT(stock_master.transaction_date, '%Y-%m-%d') AS TranDate1,
    ...
    public function reportedDistrictsByUcGraphs($wh_type, $distId, $month, $year) {
        ...
        if ($wh_type == 2) {
            ...
            $row = $this->_em_read->getConnection()->prepare($sub_qry);
            $row->execute();
            $rs = $row->fetchAll();
   ...
   SELECT sum(hf_data_master.issue_balance) as consumption,
    ...
    public function bcgCoverageReport($wh_type, $report_year, $report_month, $from_sel_month, $from_sel_year, $district, $sel_tehsil) {
        ...
        SELECT
                            ...
                            locations.pk_id,
                            sum(hf_data_detail.fixed_inside_uc_male + fixed_outside_uc_male ) as fixed_inside_uc_male,
                            sum(hf_data_detail.fixed_inside_uc_female + fixed_outside_uc_female ) as fixed_inside_uc_female,
                            sum(hf_data_detail.outreach_male + outreach_outside_male) as outreach_male,
                            sum(hf_data_detail.outreach_female + outreach_outside_female) as outreach_female,

                     (IFNULL(sum(hf_data_detail.fixed_inside_uc_male + fixed_outside_uc_male), 0) + IFNULL(sum(hf_data_detail.fixed_inside_uc_female + fixed_outside_uc_female), 0) + IFNULL(sum(hf_data_detail.outreach_male + outreach_outside_male), 0) + IFNULL(sum(hf_data_detail.outreach_female + outreach_outside_female), 0)
                        ) AS total
                    FROM
                            warehouses
                    INNER JOIN hf_data_master ON warehouses.pk_id = hf_data_master.warehouse_id
    ...
    (SELECT
        ...
        SUM(hf_data_detail.fixed_inside_uc_male + fixed_outside_uc_male) AS fixed_inside_uc_male_1,
        SUM( hf_data_detail.fixed_inside_uc_female + fixed_outside_uc_female) AS fixed_inside_uc_female_1,
        SUM(hf_data_detail.outreach_male + outreach_outside_male) AS outreach_male_1,
        SUM( hf_data_detail.outreach_female + outreach_outside_female) AS outreach_female_1,
    ...
    IFNULL(
        ...
        (
          ...
          (SELECT
                                locations.pk_id,
        sum(hf_data_detail.fixed_inside_uc_male + fixed_outside_uc_male) AS fixed_inside_uc_male_0,
        sum(hf_data_detail.fixed_inside_uc_female + fixed_outside_uc_female)  AS fixed_inside_uc_female_0,
        sum(hf_data_detail.outreach_male + outreach_outside_male) AS outreach_male_0,
        sum(hf_data_detail.outreach_female + outreach_outside_female) AS outreach_female_0,
    ...
    public function getLocationName($loc_id) {
        $loc = $this->_em->getRepository("Locations")->find($loc_id);
    ...
    public function getReportedUc1($district_id, $month, $year, $wh_type) {
        ...
        $row_1 = $this->_em_read->getConnection()->prepare($sub_qry);
     ...
     * @return type
     */
    public function ipvCoverageReport($wh_type, $report_year, $report_month, $from_sel_month, $from_sel_year, $district, $sel_tehsil) {
        if ($wh_type == 5) {
            $where = "AND tehsils.pk_id = $sel_tehsil";
        } else {
            $where = "";
        ...
        SELECT
                    ...
                    INNER JOIN locations ON warehouses.location_id = locations.pk_id
                    WHERE
                    hf_data_master.item_pack_size_id = 40 AND
                     DATE_FORMAT(
                            hf_data_master.reporting_start_date,
                            '%Y-%m'
                    ) BETWEEN '$from_report_date' AND '$report_date'
        ...
        INNER JOIN log_book_item_doses ON log_book_item_doses.log_book_id = log_book.pk_id
        WHERE
        log_book_item_doses.item_pack_size_id = 40
        AND log_book.district_id =  '$district'
     ...
     * @return string
     */
    public function receivedDate1($stock_id) {

        //changed from transaction_date/received_dat to created_date
        $str_qry1 = "SELECT
              DATE_FORMAT(MAX(stock_master.created_date), '%Y-%m-%d') AS transaction_date
    ...
    public function reverseLogisticsReport($to_wh_id) {
        $str_qry = "SELECT
	stock_master.pk_id,
	...
	fw.warehouse_name as fwh,
	stock_master.transaction_number AS trans_num,
	item_pack_sizes.item_name
FROM
	stock_master
INNER JOIN stock_detail ON stock_detail.stock_master_id = stock_master.pk_id
INNER JOIN warehouses AS fw ON stock_master.from_warehouse_id = fw.pk_id
INNER JOIN stock_batch_warehouses ON stock_detail.stock_batch_warehouse_id = stock_batch_warehouses.pk_id
INNER JOIN stock_batch ON stock_batch_warehouses.stock_batch_id = stock_batch.pk_id
INNER JOIN pack_info ON stock_batch.pack_info_id = pack_info.pk_id
INNER JOIN stakeholder_item_pack_sizes ON pack_info.stakeholder_item_pack_size_id = stakeholder_item_pack_sizes.pk_id
INNER JOIN item_pack_sizes ON stakeholder_item_pack_sizes.item_pack_size_id = item_pack_sizes.pk_id
WHERE
	stock_master.to_warehouse_id = $to_wh_id
AND stock_master.transaction_type_id = 19
GROUP BY
     ...
     * @return type
     */
    public function getReceiveQuantity($to_date, $warehouse_id) {
        $str_qry = "
             
SELECT
    stakeholder_item_pack_sizes.item_pack_size_id,
	item_pack_sizes.item_name,
	Sum(stock_detail.quantity) AS ReceivedQty
...
";
    ...
    public function getExpiredQuantity($from_date, $to_date, $warehouse_id) {
        $str_qry = "
...
";
    ...
    public function divionalTarget($sel_div, $sel_item, $year1) {
        ...
        $querypro = "SELECT
            ...
            (
            SELECT DISTINCT
            locations.location_name AS division,
            locations.pk_id as division_id,
            (
            SELECT
                IFNULL(
                        location_populations.population,
            ...
            WHERE
            locations.geo_level_id = 4
            AND locations.parent_id = '$sel_div'
            AND locations.pk_id != 10
    ...
    /**
     * Get Percent Implementation of vLMIS
     * @param type $province_id
     * @return type
     */
    public function getPercentImplementation($province_id) {

        $str_sql = "SELECT
                        A.province_id,
                        A.province_name,
                        A.totalDistricts AS totalDistricts,
                        IFNULL(B.pilotDistricts, 0) AS pilotDistricts,
                        IFNULL( ROUND( ( IFNULL(pilotDistricts, 0) / IFNULL(totalDistricts, 0) ) * 100 ), 0 ) AS percentImplementation
                    FROM
                        (
                            SELECT
                                locations.province_id,
                                province.location_name AS province_name,
                                Count(locations.pk_id) AS totalDistricts
                            FROM
                                locations
                    ...
                    LEFT JOIN (
                            SELECT
                                Count(locations.pk_id) AS pilotDistricts,
                                locations.province_id
                            FROM
                                locations
     ...
     * @return boolean
     */
    public function getDemandDetails($deman_id, $product_id) {
        $str_sql = "SELECT
        (demand_detail.approved_quantity * item_pack_sizes.number_of_doses) as approved_quantity,
        (demand_detail.demand_quantity * item_pack_sizes.number_of_doses) as demand_quantity,        
        items.pk_id
        FROM
        demand_detail
        ...
        INNER JOIN items ON item_pack_sizes.item_id = items.pk_id
        WHERE
               demand_master.pk_id=$deman_id AND items.pk_id = $product_id";
